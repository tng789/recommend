...
class BaostockOps:
    ...
    def __init__(self, home="."):
        ...
        self.very_beginning = self.calendar.very_beginning 
...
#            self.update_dataset(refresh=True)
#
#        self.total_dataset = self.total_dataset.set_index('date')
    ...
    def refresh_dataset(self, stock_list:list[str], start_date)->None:
        ...
        for code in stock_list:
            ...
            results = self._fetch_stocks(code, start_date, today_str)
            if not results.empty:
                latest_date = results.iloc[-1]['date']              #用于记录最新的日期，以便打印
                dataframes_to_concat.append(results)
                print(f"{code}  ohlcv data  {start_date} to {latest_date} pulled")
            else:
                print(f"{code}  ohlcv data  {start_date} to {today_str} not available")
                
        # 一次性合并所有数据框，然后进行去重
        final_df = pd.DataFrame()
        if dataframes_to_concat:  # 确保有数据才进行合并
            final_df = pd.concat(dataframes_to_concat, axis=0, ignore_index=True).drop_duplicates()
            # self.total_dataset = pd.concat([self.total_dataset, final_df], axis=0, ignore_index=True).drop_duplicates()

        bs.logout()
        return final_df
    def update_index(self,index_mapping, path_for_save:Path)->None:
        today = datetime.now()
...
#        }
        ...
        for name, code in index_mapping.items():
            ...
            # print(df.head())
            df.to_csv(Path(path_for_save) / name / f"{code}.csv", index=False)
        bs.logout()

    #---
    # 判断今日是否为交易日,在交易日历就绪后才可调用
    #---
...
class my_calendar:
    def __init__(self,calendar_path):
        ...
        self.calendar = self.load_calendar(calendar_path=calendar_path)
    
    def load_calendar(self, end_date:str="", calendar_path=Path("local")):
        # 登录
        if end_date == "":
        ...
        calendar = calendar_path / "calendar.csv"
        start_date = self.very_beginning
        if calendar.exists():
    ...
    def last_month(self, date:str="")->str:
        pass

    def last_day_today(self, day:datetime)->bool:
        pass
    ...
    def get_trading_days(self, start_date:str, end_date:str=""):
        ...
        df_trading_days = self.calendar[(self.calendar['calendar_date'] >= start_date) & (self.calendar['calendar_date'] <= end_date)].copy()
