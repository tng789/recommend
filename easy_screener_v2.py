...
if __name__ == "__main__":
    ...
    all_stocks_list = []
    for key, stock_list in database.stock_map.items():
        all_stocks_list += stock_list

    print(f"读取截至 {until_date_str} 的数据...")

    # 获取所有要查询的股票代码
    
    # 使用向量化操作一次性过滤所有需要的股票数据
    mask = database.total_dataset['code'].isin(all_stocks_list)
    ...
    if args.today:
        # 仅运行当天选股
        # latest_date_str = datetime.strftime(until, "%Y-%m-d%")
        print(f"执行今日选股: {until_date_str}")

        top_picks = screener.screen(all_stocks_data, until_date)
